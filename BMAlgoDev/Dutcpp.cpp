...
//     -2 - 暂无待定
/**********************************************/
IMG_VVOID DUT::VisBC_Pyramid(IMG_UBBUF src, Ipp8u *&srcRoi, IppiSize &Roi, IMG_INT level, IMG_LREAL sigma)
{
...
/**********************************************/
IMG_VVOID DUT::VisBC_LabelMarker(Ipp8u *srcRoi, IppiSize Roi, IMG_INT &markersNum)
{
...
//     -2 - 暂无待定
/**********************************************/
IMG_VVOID DUT::VisBC_OpenClose(Ipp8u *srcRoi, IppiSize Roi)
{
	//IppiSize Roi = { src.size.width,src.size.height };
...
_BACK_CODE DUT::DUTLocation(const IMG_UBBUF src, DUTOutPutParam &param, int & Angle)
{
	...
	param.cx *= 2* cbmdut.intpylevel;
	param.cy *= 2 * cbmdut.intpylevel;
	param.dist *= 2 * cbmdut.intpylevel;
	param.x1 *= 2 * cbmdut.intpylevel;
	param.x2 *= 2 * cbmdut.intpylevel;
	param.x3 *= 2* cbmdut.intpylevel;
	param.x4 *= 2* cbmdut.intpylevel;
	param.y1 *= 2* cbmdut.intpylevel;
	param.y2 *= 2* cbmdut.intpylevel;
	param.y3 *= 2* cbmdut.intpylevel;
	param.y4 *= 2* cbmdut.intpylevel;
	return status;
}

int DUT::DUTMATCH(IMG_UBBUF src, DUTOutPutParam &param, int & Angle)
{
	//缺少没有图片的判定
...
*/
	...
	for (int i = 0; i < Roi.height; i++)
	{
		for (int j = 0; j < Roi.width; j++)
		{
			if (NumMar[dstRoi[i*Roi.width + j] - 1] > (float)Roi.width*(float)Roi.height*0.30 || NumMar[dstRoi[i*Roi.width + j] - 1] < (float)Roi.width*(float)Roi.height*0.032)
	...
	if (remarkersnummic < 1 || remarkersnummicarea[tempremarkersnummicarea] < 12000 || remarkersnummicarea[tempremarkersnummicarea]>30000)
	{
		free(dstRoicopy);
		free(srcRoi);
		free(dstRoi);
	...
	//	tempheightmin < 4 || fabs(tempheightmax - Roi.height) < 4
	//	|| tempwidthmin < 4 || fabs(tempwidthmax - Roi.width) < 4
	//	)
	//{
	//	free(recodelinedstRoicopy);
	//	free(recoderowdstRoicopy);
	...
	for (int i = tempheightmin - 5; i < tempheightmax + 5; i = i + lleftrightstep)
	{
		...
		{
			if (i < 0 || j < 0 || i >= Roi.width || j >= Roi.height)
			{
				continue;
			}
	...
	//if (fabs(fabs(angleleft) - fabs(angleright)) > 5 ||
	//	fabs(fabs(angletop) - fabs(angledown)) > 5 ||
	//	fabs(fabs(angleleft) - 90) > 5 ||
	//	fabs(fabs(angleright) - 90) > 5 ||
	//	fabs(angletop) > 5 ||
	//	fabs(angledown) > 5 ||
	//	fabs(disleft + disright - 161) > 20 ||
	//	fabs(distop + disdown - 180) > 20 ||
	//	fabs(fabs(lefttopangle) - 90) > 10 ||
	//	fabs(fabs(leftdownangle) - 90) > 10 ||
	//	fabs(fabs(righttopangle) - 90) > 10 ||
	//	fabs(fabs(rightdownangle) - 90) > 10
	//	)
	//{
	//	free(recodelinedstRoicopy);
	//	free(recoderowdstRoicopy);
	...
	//if (sumpixel < 50) 
	{
		...
		for (int i = 0; i < DutRoi.height; i++)
		{
			for (int j = 0; j < DutRoi.width; j++)
			{
				if (roiroiar[dutRoi[i*DutRoi.width + j] - 1] > (float)DutRoi.width*(float)DutRoi.height*0.30 || roiroiar[dutRoi[i*DutRoi.width + j] - 1] < (float)DutRoi.width*(float)DutRoi.height*0.008
					|| (roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4])>70
					|| (roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2]) < 20

					|| (fabs((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - DutRoi.height)) < 0 && ((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4]) < 30 || (roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2]) < 20))
					|| (fabs((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2])) < 3 && ((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4]) < 30 || (roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2]) < 20))

					|| (((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4]) > 70) && ((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2]) < 40))

					|| (((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4]) > 70) && ((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2]) < 40) && (fabs((roiroinmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - DutRoi.width)) <= 0
		...
		for (int i = 0; i < DutRoi.height; i++)
		{
			for (int j = 0; j < DutRoi.width; j++)
			{
				...
				if (DutroiNumMar[dutRoi[i*DutRoi.width + j] - 1] > 1.2*temparea
					||
					DutroiNumMar[dutRoi[i*DutRoi.width + j] - 1] < 0.3*temparea
					||
					fabs(pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4]) < 10
					||
					fabs(pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2]) < 15
					||
					(fabs(pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 3] - pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 2]) > 32 && fabs(pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4 + 1] - pominmaxxy[(dutRoi[i*DutRoi.width + j] - 1) * 4]) < 40&& DutroiNumMar[(dutRoi[i*DutRoi.width + j] - 1)]<1000)
					||
		...
		if (fabs(dutcirroimaxy - dutcirroiminy) > 100)
		{
			...
			for (int i = 1; i < DutRoi.height - 1; i++)
			{
				...
				{
					dutcirroiminy = i;
					if ((dutcirroiminy + 30) < (DutRoi.height - 1))
					{
						i = dutcirroiminy + 30;
					}
					temp++;
				...
				if (temp == 2 && fabs(dutcirroimaxy - dutcirroiminy) > 20)
				{
					break;
				}
				if (temp == 2 && fabs(dutcirroimaxy - dutcirroiminy) < 20)
		...
		{
			if ((recodelinedstRoicopy[i] > 125 && recodelinedstRoicopy[i + 1] <= 125) ||
				(recodelinedstRoicopy[i] <= 125 && recodelinedstRoicopy[i + 1] > 125))
			{
				xypos.push_back(i);
			}
		}
...
int DUT::SrcLine(Ipp8u*srcRoi, IMG_SIZE roiSize, NEWLINE &line)
{
	int status = 0;
	IMG_INT threshold = 0;
	IMG_UBYTE *dstRoiE;// = NULL;
...
IMG_BOOL DUT::VisOmmLine_Fitting(IMG_RCOORD* point, NEWLINE &line, IMG_INT m)
{
	bool status = true;
	if (m < 2)
	{
	...
	do {
		...
		temp_time++;
	} while (abs(nowsum - lastsum) > 0.0002 && temp_time < 1);
...
/**********************************************/
IMG_VVOID DUT::VisBC_Fix_Sobel(IMG_UBYTE * pSrc, IppiSize roiSize, Ipp16s *&pDst, Ipp32f *&pAngle)
{
...
IMG_REAL DUT::getIntraClassVariance(Ipp16s* src, IMG_INT srcRows, IMG_INT srcCols, IMG_INT &varTh)//int &varian)
{
...
//抛物线边缘检测
IMG_INT DUT::VisEdge_detection(IMG_UBYTE *srcRoi, IMG_SIZE roiSize, IMG_INT threshold, IMG_WORD *&dstRoi, IMG_UBYTE *dstRoiE, edgeInformation *&edgeArray, IMG_INT &eNum)
{
	//如果阈值小于0，函数直接返回-1
	if (threshold < 0)
	{
...
IMG_INT DUT::VisOmmIntersection_oftwoline(FloatLine line1, FloatLine line2, IMG_RCOORD &intersectionpoint)
{
	int status = 0;
...
/**********************************************/
int DUT::Histogram(Ipp8u*srcRoi, IppiSize Roi, Ipp8u*dstRoi, double &sumpixel)
{
